...
function summarizeOrderProducts(items) {
	...
	if (!names.length) return "produktet e porosisë suaj";
	if (names.length <= 2) return names.join(", ");
	return `${names.slice(0, 2).join(", ")} dhe ${names.length - 2} produkte të tjera`;
}
...
router.get("/cart", clientOnly, async (req, res, next) => {
	if (req.user.type !== "client")
		return res.json({
			data: [],
			meta: { subtotal: 0, installation_total: 0, total: 0 },
		});
	try {
		const result = await query(
...
router.post("/cart", clientOnly, async (req, res, next) => {
	...
	try {
		...
		if (!product_id)
			throw new HttpError(422, "product_id eshte i detyrueshem.");
       ...
       returning *`,
			[
				...
				product_id,
				qty,
				Boolean(include_installation),
				Boolean(include_maintenance),
			],
		);
		res.status(201).json({ data: result.rows[0] });
	} catch (error) {
...
router.put("/cart/:id", clientOnly, async (req, res, next) => {
	...
	try {
       ...
       returning *`,
		...
		if (!result.rows[0])
			throw new HttpError(404, "Artikulli i shportes nuk u gjet.");
...
router.delete("/cart/all", clientOnly, async (req, res, next) => {
	if (req.user.type !== "client")
		return res.json({ message: "Shporta u zbraz." });
	try {
		...
		const order = await transaction(async (client) => {
         ...
         returning *`,
           ...
           returning *`,
			...
			if (clientId) {
				const reminderAt = new Date(Date.now() + 10 * 24 * 60 * 60 * 1000);
				const productSummary = summarizeOrderProducts(insertedItems);
				await client.query(
           ...
           values ($1, $2, $3, $4, $5, $6, 'pending', now())`,
					[
						...
						`Kanë kaluar 10 ditë nga porosia juaj për ${productSummary}. Hapni Mirembajtje për të plotësuar pyetësorin dhe për të kërkuar kontrollin teknik.`,
						JSON.stringify({
							screen: "mirembajtje",
							kind: "purchase_reminder",
							orderId: newOrder.id,
						}),
						reminderAt,
					],
...
router.patch("/orders/:id/status", staff, async (req, res, next) => {
	try {
		...
		if (!ORDER_STATUSES.includes(status))
			throw new HttpError(
				422,
				`Status invalid. Vlerat e vlefshme: ${ORDER_STATUSES.join(", ")}`,
			);
...
router.post("/my/warranties/activate", clientOnly, async (req, res, next) => {
	if (req.user.type !== "client")
		return next(
			new HttpError(
				422,
				"Per te aktivizuar garancine, perdor panelin e administrimit.",
			),
		);
	try {
		...
		if (!qr_code && !serial_number)
			throw new HttpError(
				422,
				"qr_code ose serial_number eshte i detyrueshem.",
			);
...
router.post("/warranties", ops, async (req, res, next) => {
	try {
		...
		if (!client_id || !product_name)
			throw new HttpError(
				422,
				"client_id dhe product_name jane te detyrueshem.",
			);

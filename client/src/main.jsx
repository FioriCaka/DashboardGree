...
function App() {
  ...
  const availableModules = useMemo(
    () => modules.filter((item) => user && (user.role === "admin" || item.roles.includes(user.role))),
    [user],
  );
    ...
    <div className={`app${sidebarOpen ? " sidebar-open" : ""}`}>
      ...
      <aside className={`sidebar${sidebarOpen ? "" : " sidebar--closed"}`}>
        ...
        <div className="sidebarFooter">
          <button className="langToggle" onClick={toggle} title={t("switchLanguage")}>
            <Globe size={16} />
      ...
      <main>
        <header className="topbar">
          <div className="topbar-left">
            <button className="menuToggle" onClick={() => setSidebarOpen((o) => !o)} title={t("toggleMenu")}>
              <Menu size={20} />
            </button>
            <div>
              <p>{t("dashboard")}</p>

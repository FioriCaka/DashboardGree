...
import { coercePayload, formatLabel, lookupOptions, normalizeInitial } from "../utils.js";
  ...
  const type =
    field.toLowerCase().includes("date") || field.endsWith("At")
      ...
      : field.toLowerCase().includes("price") ||
          ["quantity", "stock", "warranty", "inStore", "inHand", "btu", "warrantyYears", "areaSqm", "rooms"].includes(field)
        ? "number"
        : "text";
...
export default function ResourceForm({ resource, row, lookups, user, onClose, onSaved }) {
  const { t } = useLang();
  ...
  return (
    <div className="modalBackdrop">
      <form className="modal" onSubmit={submit}>
        <header>
          <h2>{row.id ? t("editTitle") : t("createTitle")} {t(resource) !== resource ? t(resource) : resource}</h2>

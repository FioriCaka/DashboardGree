...
export default function ResourcePage({ module, user }) {
  ...
  return (
    <section className="workspace">
          ...
          <thead>
            <tr>
              {module.columns.map((column) => (
                <th key={column}>{t(column) !== column ? t(column) : formatLabel(column)}</th>
              ))}
              ...
              <tr key={row.id}>
                ...
                {formFields[module.key] && (
                  <td className="actions">
                    {module.key === "products" && (
                      <button onClick={() => setViewingProduct(row)}>{t("view")}</button>
                    )}
                    <button onClick={() => setEditing(row)}>{t("edit")}</button>
                    <button className="danger" onClick={() => remove(row.id)}>{t("delete")}</button>
                  </td>
                )}
              </tr>
            ))}
      ...
      <p className="meta">
        {t("showingResults", { count: rows.length, total: meta.total ?? rows.length })}
      </p>
      ...
      {viewingProduct && (
        <ProductDetailModal
          ...
          onEdit={() => { setEditing(viewingProduct); setViewingProduct(null); }}
          onDeleted={() => { setViewingProduct(null); load(); }}
        />
      )}
    </section>
  );
